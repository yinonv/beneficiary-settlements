...
import {dataByYear, indexToGroup, Ranges, Year} from "./data/beneficiary-settlement";

const numFormatter = new Intl.NumberFormat('he-IL', {maximumSignificantDigits: 5})
...
const drawMap = (mapService: MapService, year: string) => {
    ...
    taxRanges.forEach((tr, i) => {
        ...
        target.textContent = range.min == range.max ? `${max}₪` : `${min}₪ - ${max}₪`
    });
